(() => {
    ...
    function beginCountdown() {
        ...
        showCountdown("images/count3.png", "3", false);
        countdownTimers.push(window.setTimeout(() => showCountdown("images/count2.png", "2", false), 1000));
        countdownTimers.push(window.setTimeout(() => showCountdown("images/count1.png", "1", false), 2000));
        countdownTimers.push(window.setTimeout(() => showCountdown("images/countgo.png", "Go", true), 3000));
    ...
    function spawnCoin(options = {}) {
        ...
        element.innerHTML = '<img src="images/coin.png" alt="" draggable="false">';
        playfield.appendChild(element);

        const size = playfield.clientWidth * 0.11;
        const x = xPercent === null
            ? size / 2 + Math.random() * (playfield.clientWidth - size)
            : playfield.clientWidth * (xPercent / 100);
        const speedFactor = speedMin + Math.random() * (speedMax - speedMin);
        const speed = playfield.clientHeight * speedFactor;
        const coin = {
            element,
            x,
            y: -size * (1 + ySteps),
    ...
    function spawnObstacle(mode, waveIndex) {
        const width = playfield.clientWidth * 0.18;
        ...
        element.innerHTML = '<img src="images/obstacle.png" alt="" draggable="false">';
        playfield.appendChild(element);
        ...
        const obstacle = {
            ...
            width,
            height,
            speed: playfield.clientHeight * (speedMin + Math.random() * (speedMax - speedMin)),
    ...
    function update(now) {
        ...
        coins.forEach((coin) => {
            ...
            const coinLeft = fieldRect.left + coin.x - coin.size / 2;
            const coinTop = fieldRect.top + coin.y;
            ...
            if (
                ...
                collectCoin(coin, coinLeft - fieldRect.left, coin.y);
            }
        });

        coins = coins.filter((coin) => {
            if (coin.collected) return false;
            if (coin.y > playfield.clientHeight + coin.size) {
                coin.element.remove();
                return false;
            }
            return true;
        ...
        updateObstacles(delta, now, characterRect, fieldRect);

        animationFrame = requestAnimationFrame(update);
    }

    function updateObstacles(delta, now, characterRect, fieldRect) {
        obstacles.forEach((obstacle) => {
            ...
            const obstacleLeft = fieldRect.left + obstacle.x - obstacle.width / 2;
            const obstacleTop = fieldRect.top + obstacle.y;
        ...
        obstacles = obstacles.filter((obstacle) => {
            ...
            if (obstacle.y > playfield.clientHeight + obstacle.height) {
                obstacle.element.remove();
                return false;
            }
            return true;
    ...
    function setCharacterPosition(percent) {
        const halfWidth = (character.offsetWidth / playfield.clientWidth) * 50;
